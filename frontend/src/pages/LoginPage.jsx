...
export default function LoginPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    setSubmitting(true);
    try {
      await login({ login: formData.login.trim(), password: formData.password });
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 py-16 overflow-hidden bg-slate-50">
      ...
      <div className="pointer-events-none absolute -bottom-32 -right-32 h-[420px] w-[420px] rounded-full bg-purple-200/50 blur-3xl" />
      ...
      <section className="relative mx-auto grid w-full max-w-4xl gap-8 lg:grid-cols-[1fr_1fr]">
        ...
        <div className="rounded-3xl border border-white/20 bg-white/95 p-8 shadow-2xl backdrop-blur-xl">
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="login">Email</label>
              <input
                id="login"
                name="login"
                type="email"
                ...
                className={`w-full rounded-2xl border bg-white px-4 py-3 text-slate-900 outline-none transition focus:ring-4 ${
                  fieldErrors.login ? "border-red-300 focus:border-red-500 focus:ring-red-50" : "border-slate-300 focus:border-indigo-500 focus:ring-indigo-50"
                }`}
                placeholder="alice@example.com"
                aria-invalid={Boolean(fieldErrors.login)}
              />
              {fieldErrors.login && <p className="mt-2 text-sm text-red-600">{fieldErrors.login}</p>}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  onChange={updateField}
                  className={`w-full rounded-2xl border bg-white px-4 py-3 pr-12 text-slate-900 outline-none transition focus:ring-4 ${
                    fieldErrors.password ? "border-red-300 focus:border-red-500 focus:ring-red-50" : "border-slate-300 focus:border-indigo-500 focus:ring-indigo-50"
                  }`}
          ...
          <p className="mt-6 text-sm text-slate-600">
            Need an account?{" "}
            <Link to="/register" className="font-semibold text-indigo-600 hover:text-indigo-500 transition">Register here</Link>
          </p>
        </div>

        {/* Promo panel */}
        <aside className="relative hidden items-stretch overflow-hidden rounded-3xl p-8 lg:flex border border-indigo-100 shadow-xl bg-gradient-to-br from-indigo-500 to-violet-500">
          <div className="pointer-events-none absolute -top-12 -left-12 h-40 w-40 rounded-full bg-white/20 blur-2xl" />
          <div className="pointer-events-none absolute -bottom-12 -right-12 h-52 w-52 rounded-full bg-white/15 blur-2xl" />
          ...
          <div className="relative flex w-full flex-col justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-white/20 px-4 py-2 text-sm font-semibold text-white backdrop-blur">
                <span>✨</span> SaaS-ready expense sharing
              </div>
              <h3 className="mt-5 text-4xl font-semibold leading-tight text-white">
                Split expenses.<br />Stay connected.
              </h3>
              <p className="mt-4 text-sm leading-6 text-indigo-100">
                Track shared expenses, settle balances effortlessly, and chat in real time.
              </p>
              <div className="mt-6 grid gap-3">
                {[
                  { title: "Real-time Expense Chat", sub: "Coordinate in-app without leaving the flow." },
                  { title: "Smart Balance Tracking", sub: "See net balances and who owes whom." },
                  { title: "Group Expense Management", sub: "Add members, create expenses, and settle up." },
                ].map((item) => (
